...
## API Документация: Покупка звезд (buyStars)

### Описание
API endpoint для покупки Telegram звезд для указанного пользователя. Операция выполняется через сервис Fragment с использованием токена администратора.
...
|------|-----|--------------|----------|-----------|
| stars | number | Да | Количество звезд для покупки | Минимум: 50 |
| token | string | Да | Токен администратора для авторизации | Не пустой |
| username | string | Да | Telegram username получателя звезд | Не пустой |
...
### Ответ при успехе
{
  ...
  "amountRub": "100.00",
  "starsAmount": 100,
  "recipient": "username_example",
  "finalText": "Текст сообщения с подарком"
}
...
- Неверная комиссия: "Неверная комиссия для покупки звезд, пожалуйста, обновите токен"
...
### Требования к токену
- Токен должен существовать в системе
- Минимальная комиссия для покупки звезд: 3%
- На балансе токена должно быть достаточно средств


### Примечания
- Минимальное количество звезд для покупки: 50
- Цена рассчитывается в USD с учетом комиссии токена
- Все операции логируются в системе
- При ошибках баланс не списывается
